    ...
    def queensAttacktheKing(self, queens: [[int]], king: [int]) -> [[int]]:
        ...
        for j in range(king[1]+1,8):
            if ki+','+str(j) in qc:
                ...
                break     
        for k in range(1, 8):
            if king[0]-k < 0 or king[1]-k < 0:
                break
            if str(king[0]-k)+','+str(king[1]-k) in qc:
                res.append([king[0]-k, king[1]-k])  
                break
        for k in range(1, 8):
            if king[0]+k >= 8 or king[1]+k >= 8:
                break
            if str(king[0]+k)+','+str(king[1]+k) in qc:
                res.append([king[0]+k, king[1]+k])  
                break
        for k in range(1, 8):
            if king[0]-k < 0 or king[1]+k >= 8:
                break
            if str(king[0]-k)+','+str(king[1]+k) in qc:
                res.append([king[0]-k, king[1]+k])  
                break
        for k in range(1, 8):
            if king[0]+k >= 8 or king[1]-k < 0:
                break
            if str(king[0]+k)+','+str(king[1]-k) in qc:
                res.append([king[0]+k, king[1]-k])  
                break
...
if __name__ == '__main__':
    s = Solution()
    s.queensAttacktheKing([[5,6],[7,7],[2,1],[0,7],[1,6],[5,1],[3,7],[0,3],[4,0],[1,2],[6,3],[5,0],[0,4],[2,2],[1,1],[6,4],[5,4],[0,0],[2,6],[4,5],[5,2],[1,4],[7,5],[2,3],[0,5],[4,2],[1,0],[2,7],[0,1],[4,6],[6,1],[0,6],[4,3],[1,7]],[3,4])
    s.queensAttacktheKing([[0,0],[1,1],[2,2],[3,4],[3,5],[4,4],[4,5]],[3,3])
    s.queensAttacktheKing([[0,1],[1,0],[4,0],[0,4],[3,3],[2,4]],[0,0])
