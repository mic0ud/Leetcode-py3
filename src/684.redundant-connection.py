...
class Solution:
    def findRedundantConnection(self, edges: [[int]]) -> [int]:
        connected = [i for i in range(len(edges)+1)]
        res = []
        for e in edges:
            root0 = self.find(connected, e[0])
            root1 = self.find(connected, e[1])
            if root0 == root1:
                res.append(e)
            connected[max(root0, root1)] = min(root0, root1)
        return res[-1]

    def find(self, connected: [], i) -> int:
        while connected[i] != i:
            i = connected[i]
        return i
# @lc code=end
...
if __name__ == '__main__':
    s = Solution()
    s.findRedundantConnection([[9,10],[5,8],[2,6],[1,5],[3,8],[4,9],[8,10],[4,10],[6,8],[7,9]])