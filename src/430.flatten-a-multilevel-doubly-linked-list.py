...
class Solution:
    def flatten(self, head: 'Node') -> 'Node':
        return self.flat(None, head)

    def flat(self, prev: 'Node', node: 'Node') -> 'Node':
        if not node:
            return node
        node.prev = prev	
        if node.child:
            next_ = node.next
            node.next = self.flat(node, node.child)
            node.child = None
            curr = node
            while curr.next:
                curr = curr.next
            curr.next = self.flat(curr, next_)
        else:
            node.next = self.flat(node, node.next)
        return node
# @lc code=end
