...
class Solution:
    def rankTeams(self, votes: [str]) -> str:
        rank = [defaultdict(int) for _ in range(26)]
        for vote in votes:
        ...
        def my_cmp(a, b) -> int:
            for i in range(26):
                if rank[i][a] == rank[i][b]:
                    ...
                    return rank[i][b] - rank[i][a]
            return ord(a) - ord(b)
        res = sorted(votes[0], key=cmp_to_key(my_cmp))
        return ''.join(res)
# @lc code=end
if __name__ == '__main__':
    s = Solution()
    s.rankTeams(["ABC","ACB","ABC","ACB","ACB"])
    s.rankTeams(["BCA","CAB","CBA","ABC","ACB","BAC"])
