...
class Solution:
    def makeConnected(self, n: int, connections: [[int]]) -> int:
        ...
        for a,b in connections:
            if self.find(a, root) == self.find(b, root):
                cables += 1
            else:
                self.union(a,b,root)
                islands -= 1
        res = islands - 1 if cables >= islands - 1 else -1
        return res
...
if __name__ == '__main__':
    s = Solution()
    s.makeConnected(12, [[1,5],[1,7],[1,2],[1,4],[3,7],[4,7],[3,5],[0,6],[0,1],[0,4],[2,6],[0,3],[0,2]])
