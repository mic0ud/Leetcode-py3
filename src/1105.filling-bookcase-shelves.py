...
# 
# 
# [[1,1],[2,3],[2,3],[1,1],[1,1],[1,1],[1,2]], shelf_width = 4

# @lc code=start
class Solution:
    def minHeightShelves(self, books: [[int]], shelf_width: int) -> int:
        # dp[i]: min height at books[i]
        dp = [float('inf') for _ in range(len(books)+1)]
        dp[0] = 0
        for i in range(1, len(books)+1):
            currHeight = 0
            currWidth = shelf_width
            j = i-1
            while j >= 0 and currWidth - books[j][0] >= 0:
                currWidth -= books[j][0]
                currHeight = max(currHeight, books[j][1])
                dp[i] = min(dp[i], dp[j]+currHeight)
                j -= 1
        return dp[len(books)]
# @lc code=end
