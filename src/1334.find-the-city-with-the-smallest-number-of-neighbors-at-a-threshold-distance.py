...
class Solution:
    ...
    # DFS does NOT work
    def findTheCity_WRONG(self, n: int, edges: [[int]], distanceThreshold: int) -> int:
        city_graph = [defaultdict(int) for _ in range(n)]
        for e in edges:
            city_graph[e[0]][e[1]] = e[2]
            city_graph[e[1]][e[0]] = e[2]
        cities = [0 for _ in range(n)]
        def search(i, j, d, visited):
            if visited[j] or d < 0:
                return
            cities[i] += 1
            visited[j] = True
            for k,v in city_graph[j].items():
                search(i, k, d-v, visited)
        res, count = -1, n
        for i in range(n):
            visited = [False for _ in range(n)]
            search(i, i, distanceThreshold, visited)
            if cities[i] <= count:
                count = cities[i]
                res = i
        return res
